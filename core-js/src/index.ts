...
export class Core {
  ...
  private async getDB(): Promise<any> {
    ...
    catch { throw new Error('better-sqlite3 not installed. Run: npm i better-sqlite3'); }
  ...
  async createEntry(_entry: Entry): Promise<Entry> {
    ...
    const aad = Buffer.from(`schema=${entry.schema_version}|id=${entry.id}|type=${entry.type}`);
    const payloadBuf = Buffer.from(JSON.stringify(entry.payload ?? null), 'utf8');
    const { iv, tag, ciphertext } = lwcrypto.encrypt(aad, this._sessionKey, payloadBuf);
    ...
    const tx = db.transaction((e: Entry) => {
      ...
      const tags = e.tags ?? [];
      if (tags.length) {
        const stmt = db.prepare('INSERT OR IGNORE INTO entry_tags (entry_id, tag) VALUES (?, ?)');
  ...
  async getEntry(_id: string): Promise<Entry> {
    ...
    const tags = db.prepare('SELECT tag FROM entry_tags WHERE entry_id = ?').all(_id).map((r: any) => r.tag as string);

    const aad = Buffer.from(`schema=${row.schema_version}|id=${row.id}|type=${row.type}`);
    const payloadBuf = lwcrypto.decrypt(aad, this._sessionKey, row.iv as Buffer, row.tag as Buffer, row.payload as Buffer);
    let payload: unknown = null;
    try { payload = JSON.parse(payloadBuf.toString('utf8')); } catch { payload = null; }
    const meta = row.meta_json ? JSON.parse(row.meta_json as string) : undefined;
    const e: Entry = {
      id: row.id,
      type: row.type,
  ...
  async query(_filter: QueryFilter, _pagination?: Pagination): Promise<Entry[]> {
    ...
    const where: string[] = [];
    const params: any[] = [];
    if (filter.type) { where.push('e.type = ?'); params.push(filter.type); }
    if (filter.time?.from) { where.push('e.created_at >= ?'); params.push(filter.time.from); }
    if (filter.time?.to) { where.push('e.created_at <= ?'); params.push(filter.time.to); }
    ...
    for (const row of rows) {
      const tags = db.prepare('SELECT tag FROM entry_tags WHERE entry_id = ?').all(row.id).map((r: any) => r.tag as string);
      const aad = Buffer.from(`schema=${row.schema_version}|id=${row.id}|type=${row.type}`);
      const payloadBuf = lwcrypto.decrypt(aad, this._sessionKey, row.iv as Buffer, row.tag as Buffer, row.payload as Buffer);
      let payload: unknown = null;
      try { payload = JSON.parse(payloadBuf.toString('utf8')); } catch { payload = null; }
   ...
   */
  async createProfile(dataDir: string, password: string | Buffer, params?: { N: number; r: number; p: number; keyLen?: number }): Promise<void> {
    ...
    const aad = Buffer.from(`schema=${this._schemaVersion}|type=profile`, 'utf8');
    const payload = Buffer.from(
      JSON.stringify({ magic: 'LOGWAYSS_PROFILE', schema_version: this._schemaVersion, created_at: new Date().toISOString() }),
      'utf8',
    );
    ...
    await fs.writeFile(pPath, JSON.stringify(out, null, 2), { encoding: 'utf8' });
  }

  /**
   * UnlockProfile opens and verifies the encrypted profile with the provided password.
   */
  async unlockProfile(dataDir: string, password: string | Buffer): Promise<void> {
    const pPath = this.profilePath(dataDir);
    ...
    const aad = Buffer.from(`schema=${parsed.schema_version}|type=profile`, 'utf8');
    ...
    if (obj.magic !== 'LOGWAYSS_PROFILE') throw new Error('invalid profile or password');
